...
PhysicsListMessenger::PhysicsListMessenger(ExN03PhysicsList* pPhys)
...
// fListCmd(0)
{
  fPhysDir = new G4UIdirectory("/N03/phys/");
  fPhysDir->SetGuidance("physics list commands");

	fMossbauer = new G4UIcmdWithABool("/N03/phys/mossbauer", this);
	fMossbauer->SetGuidance("Activate or deactivate the Mossbauer proccess");
	fMossbauer->SetGuidance("(by default this proccess is activated)");
	fMossbauer->AvailableForStates(G4State_PreInit);
...
void PhysicsListMessenger::SetNewValue(G4UIcommand* command,
                                          G4String newValue)
{
	if (command == fMossbauer)
		fPhysicsList->ActivateMossbauer(fMossbauer->GetNewBoolValue(newValue));
